...
async def overview(request: Request, auth:  UserDBModel = Depends(enforce_is_admin), view: str = Query(alias="ui", default="main"), page:  Union[int, None] = Query(default=1)):
    ...
    return templates.TemplateResponse("admin/overview.html", {"dt":  datetime.datetime.now(),  "users": users, "transfers": transfers, "settings":  settings, "ui": view, "user": user, "request":  request, "up": [x for x in range(1, max_users_pages + 1)], "tp": [x for x in range(1, max_tx_pages + 1)], "page": page, "lup": max_users_pages, "ltp": max_tx_pages})
